...
local fh = "/mnt/sdcard/lovegame/lua/files"
...
function FileSystem:initialize( bCache ) 
   ...
   self:mount("/", "/lua/files") -- Do not include trailing slash in paths! 
...
function FileSystem:open( sPath, sMode ) -- TODO: Compact this code 
   ...
   if not (string.sub(sPath,0,1))=="/" then
      sPath = "/"..sPath
   end
   if sMode == "r" then 
      local file, mount = self:find(sPath) 
   ...
   elseif sMode == "w" then 
      ...
      local iofile = io.open(fh .. sPath, "w")
   ...
   elseif sMode == "a" then 
      ...
      local iofile = io.open(fh .. sPath,"a")
      ...
      return handle
   elseif sMode=="n" then
         rof = io.open("/mnt/sdcard/lovegame/lua/files/trst.txt","a")
         rof:write("test worked")
         rof:close()
   end 
end 
...
function FileSystem:makeDirectory(sPath) 
   ...
   return love.filesystem.createDirectory(fh .. sPath) 
end 
...
function FileSystem:copy( fromPath, toPath ) 
   ...
   return FileSystem.copyTree(fFile, fh .. toPath) 
end 
